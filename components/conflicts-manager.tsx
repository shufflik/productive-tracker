...
export function ConflictsManager() {
  const [conflicts, setConflicts] = useState<SyncConflicts>({ goals: [], habits: [] })
  const [isOpen, setIsOpen] = useState(false)

  useEffect(() => {
    // Регистрируем обработчик конфликтов
    syncService.registerConflictsHandler((newConflicts) => {
      const hasConflicts = newConflicts.goals.length > 0 || newConflicts.habits.length > 0
      if (hasConflicts) {
  ...
  const handleResolve = (resolutions: Map<string, "local" | "server">) => {
    ...
    const allConflicts = [...conflicts.goals, ...conflicts.habits]
